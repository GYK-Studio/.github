<!-- =========================================================     
     GYK STUDIO — ORGANIZATION PROFILE
     Idioma: Español
     ========================================================= -->


<div align="center">


<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&height=210&color=0:0D1117,45:1F6FEB,100:7C3AED&text=GYK%20STUDIO&fontColor=ffffff&fontSize=48&fontAlignY=36&desc=Ideas%20%E2%86%92%20Code%20%E2%86%92%20Experiences&descAlignY=57&animation=fadeIn" alt="GYK Studio banner"/>


<img src="https://avatars.githubusercontent.com/u/191543244?s=200&v=4" width="130" alt="GYK Studio Logo"/>


<br><br>


Construyendo experiencias digitales más allá de lo ordinario.


Full-Stack · Web · Mobile · Desktop · Systems · Cloud · Gaming · Open Source


<br>


<a href="https://github.com/GYK-Studio/.github/blob/main/profile/README.md">
  <img src="https://img.shields.io/badge/Read%20in-English-1F6FEB?style=for-the-badge" alt="Read in English"/>
</a>
<a href="https://github.com/GYK-Studio">
  <img src="https://img.shields.io/badge/GitHub-GYK--Studio-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub"/>
</a>
<a href="https://github.com/orgs/GYK-Studio/repositories">
  <img src="https://img.shields.io/badge/Explorar-Repositorios-1F6FEB?style=for-the-badge&logo=github&logoColor=white" alt="Repositories"/>
</a>
<img src="https://img.shields.io/badge/Open%20Source-Projects-2EA043?style=for-the-badge&logo=opensourceinitiative&logoColor=white" alt="Open Source"/>


<br><br>


<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=19&duration=3200&pause=900&color=58A6FF&center=true&vCenter=true&width=720&lines=We+build+things+we+want+to+use.;Web%2C+mobile%2C+desktop%2C+cloud+and+gaming.;Experiment.+Build.+Improve.+Repeat." alt="Typing SVG"/>


</div>


<table align="center">
<tr>
<td align="center"><b>🌐 WEB</b></td>
<td align="center"><b>📱 MOBILE</b></td>
<td align="center"><b>🖥️ DESKTOP</b></td>
<td align="center"><b>⚙️ BACKEND</b></td>
<td align="center"><b>☁️ CLOUD</b></td>
<td align="center"><b>🎮 GAMING</b></td>
</tr>
</table>


✦ Quiénes somos


<table>
<tr>
<td width="62%" valign="top">


🚀 GYK Studio


Somos un equipo full-stack enfocado en convertir ideas en software real.


Construimos productos web, aplicaciones móviles, software de escritorio, herramientas para sistemas operativos, servicios backend, infraestructura y proyectos experimentales.


Trabajamos con tecnologías modernas sin limitarnos a un solo lenguaje, framework o plataforma.


Elegimos la tecnología según el problema — no el problema según la tecnología.


</td>
<td width="38%" valign="top">


⚡ En pocas palabras


🌐 Web & Apps
📱 Mobile
🖥️ Desktop
⚙️ Backend & APIs
☁️ Cloud & DevOps
🐳 Containers
🎮 Gaming
⛏️ Minecraft
🤖 Automation
🎨 UI / UX
🧪 Experiments


</td>
</tr>
</table>


◈ Qué construimos


<table>
<tr>


<td width="50%" valign="top">


🌐 Full-Stack Web


Productos completos desde la interfaz hasta la base de datos y el despliegue.


Frontend · Backend · APIs · Databases · Auth · Deployments


</td>


<td width="50%" valign="top">


📱 Mobile Applications


Experiencias móviles y multiplataforma para Android e iOS.


Flutter · Dart · Kotlin · Android · Cross-platform


</td>


</tr>


<tr>


<td width="50%" valign="top">


🖥️ Desktop & Operating Systems


Software de escritorio, utilidades y herramientas que interactúan directamente con sistemas operativos.


Windows · Linux · Electron · Tauri · CLI · System Tools


</td>


<td width="50%" valign="top">


⚙️ Backend & Distributed Services


APIs, autenticación, microservicios, sistemas en tiempo real y servicios internos.


REST · WebSockets · Microservices · Auth · Caching · Queues


</td>


</tr>


<tr>


<td width="50%" valign="top">


🎮 Gaming & Minecraft


Plugins, servidores, integraciones y herramientas experimentales relacionadas con gaming.


Minecraft · Spigot · Paper · Java · Server Tools


</td>


<td width="50%" valign="top">


☁️ Infrastructure & DevOps


Contenedores, redes, proxies, automatización y servicios self-hosted.


Docker · Linux · CI/CD · Reverse Proxy · Cloud · Self-hosting


</td>


</tr>


<tr>


<td width="50%" valign="top">


🤖 Automation & Developer Tools


Bots, scripts, CLIs, herramientas internas e integraciones que eliminan trabajo repetitivo.


Python · Bash · Node.js · Automation · Tooling


</td>


<td width="50%" valign="top">


🧪 Labs & Emerging Tech


Prototipos, IA, nuevos runtimes, frameworks y tecnologías que exploramos activamente.


AI · LLMs · Prototypes · Experiments · R&D


</td>


</tr>
</table>


◉ Repositorios


<div align="center">


<table>
<tr>


<td align="center" width="50%">


🚀 PROYECTOS ORIGINALES


<br>


Software creado y desarrollado originalmente por GYK Studio.


<br>


<a href="https://github.com/orgs/GYK-Studio/repositories">
<img src="https://img.shields.io/badge/VIEW-PROJECTS-238636?style=for-the-badge&logo=github&logoColor=white" alt="View projects"/>
</a>


</td>


<td align="center" width="50%">


🍴 FORKS


<br>


Repositorios derivados de proyectos externos para adaptación, experimentación, correcciones o contribuciones.


<br>


<a href="https://github.com/orgs/GYK-Studio/repositories">
<img src="https://img.shields.io/badge/VIEW-FORKS-8957E5?style=for-the-badge&logo=git&logoColor=white" alt="View forks"/>
</a>


</td>


</tr>
</table>


</div>


[!IMPORTANT]
Si un repositorio aparece en GitHub como “forked from …”, lo clasificamos como Fork, no como un proyecto original de GYK Studio.


★ Destacados

<!-- FEATURED:START -->
<table width="100%">
<thead>
<tr>
<th align="left">Proyecto</th>
<th align="left">Descripción</th>
<th align="center">Tecnologías</th>
<th align="center">Repositorio</th>
</tr>
</thead>
<tbody>
<tr>
<td valign="middle"><b>🌐 <a href="https://github.com/GYK-Studio/Web-GYK-Studio">Web GYK Studio</a></b></td>
<td valign="middle">Plataforma web oficial y portal digital de GYK Studio.</td>
<td align="center" valign="middle"><img src="https://img.shields.io/badge/Astro-1F6FEB?style=flat-square" alt="Astro"/> <img src="https://img.shields.io/badge/TypeScript-8957E5?style=flat-square" alt="TypeScript"/> <img src="https://img.shields.io/badge/Tailwind-2EA043?style=flat-square" alt="Tailwind"/></td>
<td align="center" valign="middle">
<a href="https://github.com/GYK-Studio/Web-GYK-Studio">
<img src="https://img.shields.io/badge/Explorar%20%E2%86%97-181717?style=flat-square&logo=github&logoColor=white" alt="Explorar ↗"/>
</a>
</td>
</tr>
<tr>
<td valign="middle"><b>🎬 <a href="https://github.com/GYK-Studio/NuvioWeb">Nuvio Web</a></b></td>
<td valign="middle">Experiencia multimedia TV-first para navegadores y Smart TVs.</td>
<td align="center" valign="middle"><img src="https://img.shields.io/badge/TV-First-8957E5?style=flat-square" alt="TV-First"/> <img src="https://img.shields.io/badge/JavaScript-1F6FEB?style=flat-square" alt="JavaScript"/> <img src="https://img.shields.io/badge/Streaming-2EA043?style=flat-square" alt="Streaming"/></td>
<td align="center" valign="middle">
<a href="https://github.com/GYK-Studio/NuvioWeb">
<img src="https://img.shields.io/badge/Explorar%20%E2%86%97-181717?style=flat-square&logo=github&logoColor=white" alt="Explorar ↗"/>
</a>
</td>
</tr>
<tr>
<td valign="middle"><b>⚙️ <a href="https://github.com/GYK-Studio/self-host-M">Nuvio Self-Hosted</a></b></td>
<td valign="middle">Infraestructura backend self-hosted y APIs para Nuvio.</td>
<td align="center" valign="middle"><img src="https://img.shields.io/badge/Backend-DB6D28?style=flat-square" alt="Backend"/> <img src="https://img.shields.io/badge/PostgreSQL-1F6FEB?style=flat-square" alt="PostgreSQL"/> <img src="https://img.shields.io/badge/Docker-2EA043?style=flat-square" alt="Docker"/></td>
<td align="center" valign="middle">
<a href="https://github.com/GYK-Studio/self-host-M">
<img src="https://img.shields.io/badge/Explorar%20%E2%86%97-181717?style=flat-square&logo=github&logoColor=white" alt="Explorar ↗"/>
</a>
</td>
</tr>
</tbody>
</table>
<!-- FEATURED:END -->

◇ Tecnologías


<div align="center">


🌐 Frontend


<img src="https://skillicons.dev/icons?i=html,css,js,ts,react,nextjs,vue,vite,tailwind&perline=9" alt="Tecnologías frontend"/>


HTML5 · CSS3 · JavaScript · TypeScript · React · Next.js · Vue · Vite · Tailwind CSS


<br><br>


⚙️ Backend & APIs


<img src="https://skillicons.dev/icons?i=nodejs,express,python,fastapi,java,spring,php,dotnet&perline=8" alt="Tecnologías backend"/>


Node.js · Express · Python · FastAPI · Java · Spring · PHP · .NET


<br><br>


📱 Mobile Development


<img src="https://skillicons.dev/icons?i=flutter,dart,kotlin,androidstudio&perline=8" alt="Tecnologías móviles"/>


Flutter · Dart · Kotlin · Android · Cross-platform Apps


<br><br>


🖥️ Desktop, Native & Systems


<img src="https://skillicons.dev/icons?i=electron,rust,c,cpp,cs,bash,powershell&perline=8" alt="Tecnologías de escritorio y sistemas"/>


Electron · Rust · C · C++ · C# · Bash · PowerShell · CLI Tools


<br><br>


🗄️ Databases & Data


<img src="https://skillicons.dev/icons?i=postgres,mysql,mongodb,redis,sqlite,supabase,prisma&perline=8" alt="Tecnologías de bases de datos"/>


PostgreSQL · MySQL · MongoDB · Redis · SQLite · Supabase · Prisma


<br><br>


☁️ Infrastructure, Cloud & DevOps


<img src="https://skillicons.dev/icons?i=docker,kubernetes,linux,nginx,cloudflare,githubactions,terraform,aws,gcp,azure&perline=10" alt="Tecnologías de infraestructura"/>


Docker · Kubernetes · Linux · Nginx · Cloudflare · GitHub Actions · Terraform · AWS · Google Cloud · Azure


<br><br>


🐧 Operating Systems & Environments


<img src="https://skillicons.dev/icons?i=linux,ubuntu,debian,windows,apple&perline=8" alt="Sistemas operativos"/>


Linux · Ubuntu · Debian · Windows · macOS · Servers


<br><br>


🔧 Development & Design


<img src="https://skillicons.dev/icons?i=git,github,vscode,idea,visualstudio,figma,postman&perline=8" alt="Herramientas de desarrollo"/>


Git · GitHub · VS Code · IntelliJ IDEA · Visual Studio · Figma · Postman


</div>


<br>


<table>
<tr>
<td width="33%" align="center">


🧩 Full-Stack


Frontend + backend + datos + despliegue.


</td>
<td width="33%" align="center">


📱 Multiplataforma


Web, móvil, escritorio y Smart TV.


</td>
<td width="33%" align="center">


🛠️ Sistemas


Herramientas, servidores y automatización.


</td>
</tr>
</table>


[!NOTE]
Nuestro stack no es una lista cerrada. Elegimos herramientas según cada proyecto y seguimos adoptando tecnologías modernas cuando aportan valor real.


👥 El equipo


<div align="center">


<table>
<tr>


<td align="center" width="180">
<a href="https://github.com/Kokuuuuuun">
<img src="https://github.com/Kokuuuuuun.png" width="105" alt="Kokuuuuuun"/><br>
<strong>Kokuuuuuun</strong>
</a>
<br><br>
<img src="https://img.shields.io/badge/OWNER-8957E5?style=flat-square" alt="Owner"/>
<br>
<sub>Full-Stack Developer</sub>
</td>


<td align="center" width="180">
<a href="https://github.com/yoscarmjp">
<img src="https://github.com/yoscarmjp.png" width="105" alt="yoscarmjp"/><br>
<strong>yoscarmjp</strong>
</a>
<br><br>
<img src="https://img.shields.io/badge/OWNER-8957E5?style=flat-square" alt="Owner"/>
<br>
<sub>Full-Stack Developer · Designer</sub>
</td>


<td align="center" width="180">
<a href="https://github.com/Plataer">
<img src="https://github.com/Plataer.png" width="105" alt="Plataer_"/><br>
<strong>Plataer_</strong>
</a>
<br><br>
<img src="https://img.shields.io/badge/MEMBER-238636?style=flat-square" alt="Member"/>
<br>
<sub>Full-Stack Developer · App Developer</sub>
</td>


<td align="center" width="180">
<a href="https://github.com/Anadisla">
<img src="https://github.com/Anadisla.png" width="105" alt="Anadisla"/><br>
<strong>Anadisla</strong>
</a>
<br><br>
<img src="https://img.shields.io/badge/MEMBER-238636?style=flat-square" alt="Member"/>
<br>
<sub>Full-Stack Developer</sub>
</td>


</tr>


<tr>


<td align="center" width="180">
<a href="https://github.com/AnexTurd1920">
<img src="https://github.com/AnexTurd1920.png" width="105" alt="AnexTurd1920"/><br>
<strong>AnexTurd1920</strong>
</a>
<br><br>
<img src="https://img.shields.io/badge/MEMBER-238636?style=flat-square" alt="Member"/>
<br>
<sub>Full-Stack Developer</sub>
</td>


<td align="center" width="180">
<a href="https://github.com/joelnatera113-cmd">
<img src="https://github.com/joelnatera113-cmd.png" width="105" alt="joelnatera113-cmd"/><br>
<strong>joelnatera113-cmd</strong>
</a>
<br><br>
<img src="https://img.shields.io/badge/MEMBER-238636?style=flat-square" alt="Member"/>
<br>
<sub>Full-Stack Developer</sub>
</td>


<td align="center" width="180">
<a href="https://github.com/josedarlingabreu">
<img src="https://github.com/josedarlingabreu.png" width="105" alt="josedarlingabreu"/><br>
<strong>josedarlingabreu</strong>
</a>
<br><br>
<img src="https://img.shields.io/badge/MEMBER-238636?style=flat-square" alt="Member"/>
<br>
<sub>Full-Stack Developer</sub>
</td>


<td align="center" width="180">
<img src="https://avatars.githubusercontent.com/u/191543244?s=200&v=4" width="105" alt="GYK Studio"/><br>
<strong>GYK Studio</strong>
<br><br>
<img src="https://img.shields.io/badge/BUILDING-TOGETHER-1F6FEB?style=flat-square" alt="Building together"/>
<br>
<sub>Team</sub>
</td>


</tr>
</table>


</div>


⚡ Cómo trabajamos


<table>
<tr>


<td align="center" width="25%">


01


💡 IDEA


Decidimos qué vale la pena construir.


</td>


<td align="center" width="25%">


02


🧪 BUILD


Construimos, probamos y rompemos cosas.


</td>


<td align="center" width="25%">


03


🔍 REVIEW


Revisamos código, comportamiento y diseño.


</td>


<td align="center" width="25%">


04


🚀 SHIP


Publicamos y seguimos mejorando.


</td>


</tr>
</table>


🤝 Contribuir


¿Encontraste algo que mejorar? Issues y Pull Requests son bienvenidos cuando el repositorio los acepte.


<table>
<tr>


<td align="center" width="25%">
<strong>① Fork</strong><br>
<sub>Crea tu copia</sub>
</td>


<td align="center" width="25%">
<strong>② Branch</strong><br>
<sub>Trabaja aislado</sub>
</td>


<td align="center" width="25%">
<strong>③ Commit</strong><br>
<sub>Describe el cambio</sub>
</td>


<td align="center" width="25%">
<strong>④ Pull Request</strong><br>
<sub>Propón tu mejora</sub>
</td>


</tr>
</table>


git checkout -b feature/my-feature
git add .
git commit -m "feat: add my feature"
git push origin feature/my-feature


<details>
<summary><strong>📌 Convenciones de commits</strong></summary>


<br>


Prefijo


Uso


feat


Nueva funcionalidad


fix


Corrección de errores


docs


Documentación


refactor


Refactorización


perf


Rendimiento


test


Pruebas


chore


Mantenimiento


ci


CI / automatización


</details>


📚 Más información


<details>
<summary><strong>🧭 Principios</strong></summary>


<br>










🧠 Aprender construyendo


Aprendemos creando cosas reales.


🧪 Experimentar


Experimentar forma parte del proceso.


🤝 Construir juntos


Compartimos conocimiento y revisamos nuestro trabajo.


🔓 Abierto cuando sea posible


Compartimos código cuando el proyecto lo permite.


📈 Seguir mejorando


Ninguna versión tiene que ser la última.


</details>


<details>
<summary><strong>🍴 Política de Forks</strong></summary>


<br>


Un repositorio derivado de otro proyecto se identifica como Fork.


Los forks pueden existir para:


adaptar software a nuestras necesidades;


probar cambios;


preparar contribuciones;


corregir problemas;


mantener una variante personalizada;


estudiar o experimentar con una implementación.


El crédito, historial y licencia del proyecto original deben conservarse cuando corresponda.


</details>


<details>
<summary><strong>🔐 Seguridad</strong></summary>


<br>


Si encuentras una vulnerabilidad, evita publicar información sensible directamente en un Issue público.


Cuando un repositorio tenga una sección Security, sigue el proceso indicado allí.


</details>


<details>
<summary><strong>📄 Licencias</strong></summary>


<br>


Cada proyecto puede utilizar una licencia diferente.


Consulta siempre el archivo LICENSE del repositorio antes de copiar, modificar o redistribuir código.


Los forks continúan sujetos a las condiciones de sus proyectos originales.


</details>
---


<div align="center">


✦ Explora. Construye. Mejora.


<a href="https://github.com/GYK-Studio">
<img src="https://img.shields.io/badge/GYK%20STUDIO-181717?style=for-the-badge&logo=github&logoColor=white" alt="GYK Studio"/>
</a>


<br><br>


Hecho con código, curiosidad y trabajo en equipo. 🇩🇴


<br>


<sub>GYK Studio · Perfil de organización en GitHub</sub>


<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&height=120&section=footer&color=0:0D1117,45:1F6FEB,100:7C3AED" alt="Footer"/>


</div>
